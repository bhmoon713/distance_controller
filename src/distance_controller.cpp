...
public:
  DistanceController() : Node("distance_controller") {
    ...
    relative_goals = {{0.0, 1.0, +0.00000},  {0.0, -1.0, +0.00000},
                      {0.0, -1.0, +0.00000}, {0.0, 1.0, +0.00000},
                      {1.0, 1.0, +0.00000},  {-1.0, -1.0, +0.00000},
                      {1.0, -1.0, +0.00000}, {-1.0, 1.0, +0.00000},
                      {1.0, 0.0, +0.00000},  {-1.0, 0.0, +0.00000}};
...
private:
  ...
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    if (!initialized) {
      RCLCPP_INFO(this->get_logger(), "Initial position: (%.3f, %.3f)",
                  current_x, current_y);
      initialized = true;
    }
  }
  ...
  void controlLoop() {
    ...
    RCLCPP_INFO(this->get_logger(),
                "delta X: %.3f, delta Y: %.3f, Distance: %.3f", error_x,
                error_y, distance);
    ...
    if (distance < goal_tolerance) {
      RCLCPP_INFO(this->get_logger(), "Relative Goal %zu reached",
                  current_goal_index);
